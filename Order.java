...
public class Order extends JFrame implements ActionListener
{
	...
	public Order(Dealer d)
	{
		...
		String[] columns = {"TreeID","DealerID", "UserID","Quantity"};
		...
		else if(text.equals(confirm.getText()))
		{
			...
			for (int i = 0; i < selectedRow.length; i++) 
			{
			  for (int j = 0; j < selectedColumns.length; j++) 
			  {
				...
				selectedData = (String) table.getValueAt(selectedRow[i], selectedColumns[i]);
				System.out.println(selectedData);
			  }
			}
			
			System.out.println("User ID is "+selectedData);
	...
	public Object[][] getTableDetails()
	{
		...
		try
		{
			...
			connection=DatabaseConnection.DbConnector();
			String mysql="SELECT `TreeID` , `DealerID` , `UserID`, `Quantity` FROM `order` WHERE `order`.`DealerID` = '"+DealerId+"'";
			System.out.println("Dealer id is"+ DealerId);
			Statement st = connection.createStatement();
			System.out.println("statement created");
